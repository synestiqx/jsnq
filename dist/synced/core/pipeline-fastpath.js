import { criteriaMatch } from './match.js';
import { compileCriteriaPredicate } from './compiled-predicate.js';
import { compileFlatMutation } from './compiled-mutation.js';
import { applyValueAction, isValueAction, prepareActions } from './actions.js';
import { cloneJson, getBySegments, splitPath } from './utils.js';
import { createJsonPathPlanFromSegments } from './data-engine.js';
import { hasNestedCriterionCandidate } from './flat-array-fast-path.js';
...
export function collectPipelineIntent(ops) {
 ...
 */
export function collectFlatValueActionPaths(currentValue, ops) {
    ...
    if (hasNestedCriterionCandidate(currentValue, intent.criteria, FASTPATH_OPTIONS))
        return null;
    const strictCtx = { warnedUnknownOps: new Set(), warnings: [] };
    const paths = [];
    for (let index = 0; index < currentValue.length; index++) {
        if (!criteriaMatch(intent.criteria, currentValue[index], FASTPATH_OPTIONS, strictCtx))
            continue;
...
function preciseActionKeys(actions) {
    ...
    for (const action of actions) {
        ...
        if (typeof key !== 'string' || key.length === 0 || splitPath(key).length !== 1)
            return null;
        keys.push(key);
    }
    return keys;
 ...
 */
export function tryFastPipelineMutation(currentValue, ops, options = {}) {
    const intent = collectPipelineIntent(ops);
    ...
    const compiledMutation = compileFlatMutation(intent.criteria, intent.actions);
    if (compiledMutation) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        for (const action of prepared) {
            if (applyValueAction(clone, action, FASTPATH_OPTIONS, stats))
                mutations++;
        }
        next[i] = clone;
        if (affectedPaths && preciseKeys)
 ...
 */
export function applyInsertToInsideArrayCow(currentValue, position, data) {
 ...
 */
export function tryFastStructuralMutation(currentValue, intent) {
    if (intent.optionsTouched)
        return undefined;
...
export function isDeepSugarAction(action) {
    const a = action;
 ...
 */
export function applyDeepSugarPatch(current, criteria, actions) {
    if (current === null || typeof current !== 'object')
        return current;
    const compiledCriteria = criteria;
    const strictCtx = { warnedUnknownOps: new Set(), warnings: [] };
    if (!criteriaMatch(compiledCriteria, current, FASTPATH_OPTIONS, strictCtx))
        return current;
