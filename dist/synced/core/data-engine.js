...
export function setJsonPlanCacheLimit(limit) {
...
export function getJsonPlanCacheStats() {
...
export function clearJsonPlanCache() {
    planCache.clear();
...
export function createJsonPathPlan(path) {
...
export function createJsonPathPlanFromSegments(segments) {
...
export function getJsonParentSegments(pathOrPlan) {
    const plan = typeof pathOrPlan === 'string' ? createJsonPathPlan(pathOrPlan) : pathOrPlan;
    return plan.parentSegments;
}
export function getJsonAffectedPaths(pathOrPlan, mode = 'exact') {
    const plan = typeof pathOrPlan === 'string' ? createJsonPathPlan(pathOrPlan) : pathOrPlan;
...
export function createMutationResult(init) {
...
export function readJsonPath(root, pathOrPlan) {
    const plan = typeof pathOrPlan === 'string' ? createJsonPathPlan(pathOrPlan) : pathOrPlan;
    return plan.segments.length === 0 ? root : getJsonBySegments(root, plan.segments);
}
export function getJsonBySegments(obj, segments) {
...
export function hasJsonPath(root, pathOrPlan) {
    const plan = typeof pathOrPlan === 'string' ? createJsonPathPlan(pathOrPlan) : pathOrPlan;
    let current = root;
...
export function resolveJsonParentAndKey(root, pathOrPlan, options = {}) {
    const plan = typeof pathOrPlan === 'string' ? createJsonPathPlan(pathOrPlan) : pathOrPlan;
    if (plan.segments.length === 0)
...
export function writeJsonPath(root, pathOrPlan, value) {
    const plan = typeof pathOrPlan === 'string' ? createJsonPathPlan(pathOrPlan) : pathOrPlan;
    if (plan.key == null) {
 ...
 */
export function writeJsonPathValue(root, pathOrPlan, value) {
    const plan = typeof pathOrPlan === 'string' ? createJsonPathPlan(pathOrPlan) : pathOrPlan;
    if (plan.key == null)
        return false;
    const { parent, key } = resolveJsonParentAndKey(root, plan, { create: true });
...
export function deleteJsonPath(root, pathOrPlan) {
    const plan = typeof pathOrPlan === 'string' ? createJsonPathPlan(pathOrPlan) : pathOrPlan;
    if (plan.key == null) {
