import { isOperatorKnown } from './operators-registry.js';
let canCompile = null;
...
let cacheMax = 2000;
export function setCompiledPredicateCacheLimit(limit) { cacheMax = Math.max(0, limit | 0); }
export function clearCompiledPredicateCache() { factoryCache.clear(); }
...
function isCodegenable(criteria) {
    ...
    for (const c of criteria) {
        ...
        if (c.segments[0] === undefined)
            return false;
        if (!isOperatorKnown(String(c.operator)))
            return false;
 ...
 */
export function compileCriteriaPredicate(criteria) {
