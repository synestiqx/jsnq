import { getOperatorFn, isOperatorKnown } from './operators-registry.js';
import { deepArrayMatch, getBySegments, isObject, parseDeepSearchPath, splitPath } from './utils.js';
export function compileCriterion(key, operator, value) {
...
export function enforceKnownOperator(criterion, options, ctx) {
 ...
 */
export function criterionMatches(criterion, data) {
 ...
 */
export function criteriaMatch(criteria, data, options, ctx) {
    for (let i = 0; i < criteria.length; i++) {
        const c = criteria[i];
        enforceKnownOperator(c, options, ctx);
        if (c.isDeep) {
            if (!deepArrayMatch(data, c.deepArrayKey, c.segments, c.opFn, c.value, options.maxDepth ?? 10))
                return false;
            continue;
        }
