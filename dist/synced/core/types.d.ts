export type ComparisonOperator = '==' | '===' | '!=' | '!==' | '>' | '>=' | '<' | '<=' | 'includes' | '!includes' | 'startsWith' | 'endsWith' | 'regex' | 'isArray' | 'isObject' | (string & {});
export type Primitive = string | number | boolean | null | undefined;
export type JsonLike = Primitive | JsonLike[] | {
    [k: string]: JsonLike;
};
...
type PathRec<T, D extends number> = T extends ReadonlyArray<infer U> | (infer U)[] ? // array: numeric or bracketed index, optionally followed by nested path
ArrayPathSegment<D> | NestedArrayPath<U, D> : T extends object ? {
    [K in Extract<keyof T, string>]: ObjectPathSegment<K, T[K], D>;
...
type PathValueArrayCase<T, K extends string, R extends string> = K extends `${number}` ? T extends ReadonlyArray<infer U> | (infer U)[] ? PathValueRec<U, R> : unknown : K extends keyof T ? PathValueRec<T[K], R> : unknown;
type PathValueBaseCase<T, K extends string> = K extends `${number}` ? T extends ReadonlyArray<infer U> | (infer U)[] ? U : unknown : K extends keyof T ? T[K] : unknown;
type PathValueRec<T, P extends string> = P extends `${infer K}.${infer R}` ? PathValueArrayCase<T, K, R> : P extends `${infer K}` ? PathValueBaseCase<T, K> : unknown;
...
export type OperatorFor<V> = V extends number ? ComparisonOps | PatternOps : V extends string ? ComparisonOps | PatternOps : V extends boolean ? EqualityOps : V extends ReadonlyArray<unknown> | unknown[] ? ComparisonOps | PatternOps : EqualityOps | TypeCheckOps | (string & {});
...
export type ActionType = 'replace' | 'update' | 'merge_update' | 'delete_key' | 'delete_element' | 'insert' | 'insert_to' | 'move' | 'move_matches' | 'move_matches_overwrite' | 'copy' | 'copy_matches' | 'move_first_to_matches' | 'copy_first_to_matches';
/**
...
export type Action = ReplaceAction | UpdateAction | MergeUpdateAction | DeleteKeyAction | DeleteElementAction | InsertAction | InsertToAction | MoveAction | MoveMatchesAction | MoveMatchesOverwriteAction | CopyAction | CopyMatchesAction | MoveFirstToMatchesAction | CopyFirstToMatchesAction;
export interface SearchOptions {
    ...
    buildMeta?: boolean;
    returnPaths?: boolean;
    immutable?: boolean | 'auto';
    dryRun?: boolean;
    strictPathsWarn?: boolean;
    operatorsStrict?: 'warn' | 'throw';
    arrayMergeStrategy?: ArrayMergeStrategy;
    arrayMergeKey?: string | ((x: unknown) => string | number);
    overwritePolicy?: 'overwrite' | 'skip' | 'error';
    warnOnOverwrite?: boolean;
    objectOrderWarning?: boolean;
    /**
...
type BracketPathRec<T, D extends number> = T extends ReadonlyArray<infer U> | (infer U)[] ? `[${number}]` | (D extends 0 ? never : `[${number}].${BracketPathRec<U, _Prev[D]>}`) : T extends object ? {
    [K in Extract<keyof T, string>]: `${K}` | (D extends 0 ? never : `${K}.${BracketPathRec<T[K], _Prev[D]>}`);
}[Extract<keyof T, string>] : never;
