import { dfsIterator, scanJsonMatches, getBySegments, isObject, isRecordObject, resolveTargetWithPathCreation, hasPath, cloneJson, deepArrayIterator } from './utils.js';
import { criteriaMatch, criterionMatches, enforceKnownOperator } from './match.js';
import { compileCriteriaPredicate } from './compiled-predicate.js';
import { applyValueAction, prepareActions } from './actions.js';
import { assertCanInsertIntoTargetPath, assignWithPolicy, canInsertIntoResolvedTarget, canInsertRelative, canRemoveFromOriginal, getAssignmentEffect, insertIntoTargetPath, insertRelative, removeFromOriginal, selectTargets, fanoutMatchesToTargets, orderMatchesForMove, wouldCreateMoveCycle, wouldCreateMoveCycleAtPath } from './ops.js';
import { executeFlatArrayFastPath } from './flat-array-fast-path.js';
...
export class JsnqPipeline {
        ...
        try {
            ...
            if (hasDeepArrayCriteria) {
                ...
                const scan = scanJsonMatches(this.data, {
                    ...
                    buildMeta: needMeta,
                    returnPaths: needPaths,
                }, (node) => this.criteria.length === 0 || (compiledPred ? compiledPred(node) : criteriaMatch(this.criteria, node, this.options, this.strictCtx)), (node) => {
        ...
        for (let i = 0; i < this.criteria.length; i++) {
            ...
            for (const currentNode of currentNodes) {
                if (c.isDeep && c.deepArrayKey) {
                    ...
                    for (const deepNode of deepArrayIterator(currentNode.data, c.deepArrayKey, c.segments, c.opFn, c.value, currentNode.path || [], currentNode.depth, this.options.maxDepth ?? 10)) {
    ...
    applyMoveOrCopy(node, action, kind) {
        ...
        if (!this.options.dryRun) {
            ...
            insertIntoTargetPath(this.data, position, element, mode, () => plannedTarget, key, this.options, this.stats);
        }
    ...
    applyMatchFanout(action, fan, matches) {
        ...
        const targetsAll = selectTargets(this.data, this.options, targetKey, targetOperator, targetValue, (mode ?? 'inside') !== 'inside');
        const targets = fan.allTargets ? targetsAll : (targetsAll.length ? [targetsAll[0]] : []);
        const applied = fanoutMatchesToTargets(fan.kind, matches, targets, mode, key, this.options, this.stats, !!this.options.dryRun);
        if (fan.kind === 'move')
    ...
    applyMoveMatchesOverwrite(action, matches) {
        ...
        for (const src of ordered) {
            ...
            const writableTargets = objectTargets.filter((target) => {
                if (wouldCreateMoveCycle(src.data, target, 'inside'))
                    return false;
                const targetData = target.data;
                const exists = overwriteKey in targetData;
                const effect = getAssignmentEffect(targetData, overwriteKey, this.options, (conflictKey) => new Error(`copy/move overwrite prevented for key '${conflictKey}'`));
            ...
            if (!this.options.dryRun) {
                ...
                for (let i = 0; i < writableTargets.length; i++) {
                    ...
                    assignWithPolicy(t.data, overwriteKey, i === 0 ? src.data : cloneJson(src.data), this.options, this.stats, (conflictKey) => new Error(`copy/move overwrite prevented for key '${conflictKey}'`));
                }
            }
            this.stats.moved++;
            if (this.options.trackOperations !== false)
