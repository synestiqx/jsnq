import { cloneJson, isObject } from './utils.js';
...
import { insertRelative } from './ops.js';
export function executeFlatArrayFastPath(params) {
    ...
    const compiledMutation = !hasLimit && !isDeleteElementOnly && !relativeInsert
        ? compileFlatMutation(params.criteria, params.actions)
        : null;
    if (compiledMutation) {
    ...
    for (let index = 0; index < items.length; index++) {
        ...
        params.stats.nodesVisited++;
        params.stats.maxDepth = Math.max(params.stats.maxDepth, 1);
        if (!(pred ? pred(item) : criteriaMatch(params.criteria, item, params.options, strictCtx)))
            continue;
 ...
 */
export function hasNestedCriterionCandidate(items, criteria, options) {
