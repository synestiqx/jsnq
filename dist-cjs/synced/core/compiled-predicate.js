...
exports.setCompiledPredicateCacheLimit = setCompiledPredicateCacheLimit;
exports.clearCompiledPredicateCache = clearCompiledPredicateCache;
exports.opExpr = opExpr;
exports.compileCriteriaPredicate = compileCriteriaPredicate;
...
let cacheMax = 2000;
function setCompiledPredicateCacheLimit(limit) { cacheMax = Math.max(0, limit | 0); }
function clearCompiledPredicateCache() { factoryCache.clear(); }
...
function isCodegenable(criteria) {
    ...
    for (const c of criteria) {
        ...
        if (!(0, operators_registry_1.isOperatorKnown)(String(c.operator)))
            return false;
        if (opExpr(String(c.operator), 'a', 'b') === null)
            return false;
    }
    return true;
}
function buildFactory(criteria) {
    const lines = [
 ...
 */
function compileCriteriaPredicate(criteria) {
    if (!compilationAvailable() || !isCodegenable(criteria))
        return null;
    const sig = criteria
...
//# sourceMappingURL=compiled-predicate.js.map